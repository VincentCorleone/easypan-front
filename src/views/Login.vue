...
const captchaUrlEmail = ref(
  baseUrl + api.captcha + "?type=email&time=" + new Date().getTime()
);
...
const validatorEmailCode = (rule, value, callback) => {
  ...
  if (!pattern.test(value))
    return callback(new Error("邮箱验证码格式不正确，正确格式为6位数字"));
  callback();
};
...
const captcha = (rule, value, callback) => {
  ...
  if (!pattern.test(value))
    return callback(new Error("图片验证码格式不正确，正确格式为5位密码或数字"));
  callback();
};
...
function userInfo() {
  proxy.Request.get("/user/info").then((response) => {
  ...
  <div class="login-bg">
    ...
    <div class="login-panel">
      ...
      >
          ...
          <div class="checkCodeWrapper">
            ...
            <div class="getEmailCode">
              <el-button
                type="primary"
                size="large"
                @click="openEmailSender('/sendEmailCodeForResetPassword')"
                >获取验证码</el-button
              >
            </div>
          </div>
        ...
        <el-form-item>
          <el-button class="login-btn" type="primary" @click="resetPassword()"
            >重置密码</el-button
          >
        </el-form-item>
      </el-form>
          ...
          <div class="checkCodeWrapper">
            ...
            <div class="getEmailCode">
              <el-button
                type="primary"
                size="large"
                @click="openEmailSender('/sendEmailCodeForRegister')"
                >获取验证码</el-button
              >
            </div>
          </div>
